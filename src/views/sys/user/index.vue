  ...
  <div class="app-container">
      ...
      <el-table
        ...
        <template v-slot="scope" >
          ...
          <el-popconfirm  v-if="scope.row.username!='rbac'" title="您确定要对这个用户重置密码吗？" @confirm="handleResetPassword(scope.row.id)">
            <template #reference>
              <el-button  type="warning" :icon="RefreshRight" v-if="hasAuth('system:user:resetPwd')">重置密码</el-button>
            </template>
          </el-popconfirm>

          <el-button v-if="scope.row.username!='rbac' && hasAuth('system:user:edit')" type="primary" :icon="Edit" @click="handleDialogValue(scope.row.id)" />

            <el-popconfirm  v-if="scope.row.username!='rbac'" title="您确定要删除这条记录吗？" @confirm="handleDelete(scope.row.id)">
              <template #reference>
